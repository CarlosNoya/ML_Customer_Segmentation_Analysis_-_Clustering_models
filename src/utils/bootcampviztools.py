...
def plot_grouped_histograms(df, cat_col, num_col, group_size, color = None):
    unique_cats = df[cat_col].unique()
    num_cats = len(unique_cats)
    ...
    for i in range(0, num_cats, group_size):
        ...
        for cat in subset_cats:
            sns.histplot(subset_df[subset_df[cat_col] == cat][num_col], kde=True, label=str(cat), palette = color)
...
def plot_features_num_regression(df, target_col="", columns=[], umbral_corr=0, pvalue=None):
    ...
    for i in range(num_groups):
        ...
        # Generar el pairplot
        sns.pairplot(df[subset], diag_kind="kde", corner=True, hue=target_col)
        plt.show()
